...
export default async function EvidenceDetailPage({
  ...
  const evidence = getEvidenceBySlug(slug, evidenceSlug);

  if (!caseRecord || !evidence) {
    notFound();
  }

  const relatedEntities = getRelatedEntities(slug, evidence.relatedEntitySlugs);
      ...
      <section className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <article className="rounded-[2rem] border border-white/10 bg-slate-950/35 p-6">
          ...
          <div className="mt-4 flex flex-wrap gap-3 text-sm text-slate-300">
            ...
            <span>{formatCaseDate(evidence.sortDate)}</span>
          </div>
        ...
        <aside className="space-y-6">
          <div className="rounded-[2rem] border border-white/10 bg-white/5 p-6">
            ...
            <p className="mt-2 text-sm leading-7 text-slate-300">
              Discovery phase {evidence.discoveryPhase}
            </p>
          </div>
